...
function FilteredScienceTeachers({seeAll, scienceTeachers}) {
    ...
    var filteredMusic = scienceTeachers.filter(function (lesson) {
        return lesson.category.includes('music')

    });

    var filteredEnglish = scienceTeachers.filter(function (lesson) {
        return lesson.category.includes('english')

    });

    var filteredScience = scienceTeachers.filter(function (lesson) {
        return lesson.category.includes('science')

    });

    var filteredMaths = scienceTeachers.filter(function (lesson) {
                ...
                seeAll ? (!selectedTabMaths ?
                    ...
                    (!selectedTabMaths ?
                        !selectedTabScience ?
                            !selectedTabEnglish ?
                                !selectedTabMusic ?
                                scienceTeachers.slice(0, 3).map((mathCard) => {
...
export default FilteredScienceTeachers
